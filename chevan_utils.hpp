...
#undef v4
  ...
  namespace chevanut_perlin_noise
  {
    ...
    static inline int32_t PerlinNoise2(int32_t x, int32_t y) { return global_perlin_noise_hash[(global_perlin_noise_hash[(y + global_perlin_noise_seed) % 256] + x) % 256]; }
    static inline float PerlinLinearlyInterpolate(float x, float y, float s) { return x + s * (y - x); }
    static inline float PerlinSmoothlyInterpolate(float x, float y, float s) { return PerlinLinearlyInterpolate(x, y, s * s * (3 - 2 * s)); }
