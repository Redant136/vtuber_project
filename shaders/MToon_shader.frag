...
void main()
{
    ...
    vec3 lightDir=lightSource.xyz-Pos;
    float lightIntensity=max(dot(normalize(normal),normalize(lightDir)),0.f)*lightSource.w;
    ...
    color.xyz = lerp(shadeColor,materialColor,lightIntensity).xyz*color.xyz;
